...
template <typename T>
void addVectorToVectorUniquely(std::vector<T>& aVs, const std::vector<T>& aVsAdd)
{
...
template <typename T>
void addValueToVectorUniquely(std::vector<T>& aVs, T oT)
{
	const auto itFind = std::find(aVs.begin(), aVs.end(), oT);
	if (itFind == aVs.end()) {
...
template <typename T>
void addValueToDequeUniquely(std::deque<T>& aVs, T oT)
{
	const auto itFind = std::find(aVs.begin(), aVs.end(), oT);
	if (itFind == aVs.end()) {
		aVs.push_back(std::move(oT));
...
template <typename T>
bool isInVector(std::vector<T>& aVs, const T& oT)
{
	const auto itFind = std::find(aVs.begin(), aVs.end(), oT);
...
// expects sPath == cleanupPath(sPath)
std::string getPathFromDirAndName(const std::string& sDirPath, const std::string& sName);
...
{
public:
	static FileStat create(const std::string& sPath);
	bool exists() const { return ((m_nFStat & FILE_STAT_EXISTS) == FILE_STAT_EXISTS); }
	bool isRegular() const { return ((m_nFStat & FILE_STAT_IS_REGULAR) == FILE_STAT_IS_REGULAR); }
	bool isDir() const { return ((m_nFStat & FILE_STAT_IS_DIR) == FILE_STAT_IS_DIR); }
	/** Whether the file is a symbolic link.
	 * Implies isRegular() and !isDir(). Might be dangling.
	 * @return Whether sym link.
	 */
	bool isSymLink() const { return ((m_nFStat & FILE_STAT_IS_SYM_LINK) == FILE_STAT_IS_SYM_LINK); }
private:
