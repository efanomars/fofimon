...
int32_t INotifierSource::getSystemMaxUserWatches()
{
...
INotifierSource::INotifierSource(int32_t nReserveSize)
: Glib::Source()
, m_nINotifyFD(-1)
{
...
INotifierSource::~INotifierSource()
{
	if (m_nINotifyFD != -1) {
		::close(m_nINotifyFD);
	}
...
void INotifierSource::attach_override()
{
...
int32_t INotifierSource::findEntryByTag(int32_t nTag)
{
...
int32_t INotifierSource::findEntryByWatch(int32_t nWatchDescriptor)
{
	const auto itFind = std::find_if(m_aWatchItems.begin(), m_aWatchItems.end(), [&](const WatchItem& oWI)
	{
...
template <int32_t N>
bool startsWithAnyOf(const std::string& sPath, std::array<const char*, N>& aStarters)
{
...
std::vector<std::string> INotifierSource::invalidPaths()
{
...
std::pair<int32_t, int32_t> INotifierSource::addPath(const std::string& sPath, int32_t nTag)
{
...
int32_t INotifierSource::clearAll()
{
	int32_t nErrno = 0;
...
int32_t INotifierSource::removePath(int32_t nTag)
{
	return removePath(-1, nTag);
}
int32_t INotifierSource::removePath(int32_t nWatchIdx, int32_t nTag)
{
	assert(nWatchIdx >= -1);
	#ifndef NDEBUG
...
int32_t INotifierSource::renamePath(int32_t nFromTag, int32_t nToTag)
{
	return renamePath(-1, nFromTag, nToTag);
}
int32_t INotifierSource::renamePath(int32_t nFromWatchIdx, int32_t nFromTag, int32_t nToTag)
{
...
sigc::connection INotifierSource::connect(const sigc::slot<FOFI_PROGRESS, const FofiData&>& oSlot)
{
	if (m_nINotifyFD == -1) {
...
bool INotifierSource::prepare(int& nTimeout)
{
	nTimeout = -1;

	return false;
}
bool INotifierSource::check()
{
	bool bRet = false;
...
bool INotifierSource::dispatch(sigc::slot_base* p0Slot)
{
