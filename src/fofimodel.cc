...
//	const bool bIsNotSymLink = bExists && !oFStat.isSymLink();
...
//			const bool bIsNotSymLink = bExists && !oFStat.isSymLink();
			const bool bExistsAndDir = bExists && oFStat.isDir();
//std::cout << "FofiModel::initialCreateToWatchDir  sChildPath=" << sChildPath << "  bExistsAndDir=" << bExistsAndDir << " nFStat=" << nFStat << '\n';
			if (! bExistsAndDir) {
				continue; //-----
			}
