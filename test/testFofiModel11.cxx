...
int testDeepRenamesTransitive()
{
	...
	ForkingFixture oForkingFixture([&](){
		...
		oTempFileTreeFixture.renameRelPathName("A2", "A3");
	});
...
//std::cout << "  oTempFileTreeFixture.m_sTestBasePath=" << oTempFileTreeFixture.m_sTestBasePath << '\n';
	EXPECT_TRUE(aResults.size() == 3 * (7 + (7-1) * nGenFiles));
...
//std::cout << "         oResult.m_aActions.size()=" << oResult.m_aActions.size() << '\n';
		...
		if (sPathName.find(oTempFileTreeFixture.m_sTestBasePath + "/A1", 0) != std::string::npos) {
			EXPECT_TRUE(oResult.m_eResultType == FofiModel::RESULT_DELETED);
			EXPECT_TRUE(oResult.m_aActions.size() == 1);
			EXPECT_TRUE(oResult.m_aActions[0].m_eAction == INotifierSource::FOFI_ACTION_RENAME_FROM);
			++nCountA1;
		} else if (sPathName.find(oTempFileTreeFixture.m_sTestBasePath + "/A2", 0) != std::string::npos) {
...
//std::cout << "  nCountA1=" << nCountA1 << '\n';
	EXPECT_TRUE(nCountA1 == nCountA2);
	EXPECT_TRUE(nCountA1 == nCountA3);
	EXPECT_TRUE(nCountA1 * 3 == aResults.size());
	return 0;
}
...
{
	std::cout << "FofiModel11 Tests:" << '\n';

	EXECUTE_TEST(fofi::testing::testDeepRenamesTransitive());
	//
	std::cout << "FofiModel11 Tests successful!" << '\n';
	return 0;
}
