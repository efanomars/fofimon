...
int testDontFollowSymLinks()
{
	...
	oTempFileTreeFixture.makeRelSymlink("A2", false, "A1/B11/C111");
	...
	ForkingFixture oForkingFixture([&](){
		...
		oTempFileTreeFixture.makeRelSymlink("A3", false, "A1/B12");
	});
...
//std::cout << "  oToWatch.m_sPath=" << oToWatch.m_sPathName << "  oToWatch.m_nDepth=" << oToWatch.m_nDepth << "  zone=" << oToWatch.getOwnerDirectoryZone() << '\n';
//}
...
//std::cout << "   oResult0.m_aActions.size()=" << oResult0.m_aActions.size() << '\n';
	...
	EXPECT_TRUE(oResult1.m_sName == "A3");
//std::cout << "   oResult1.m_aActions.size()=" << oResult1.m_aActions.size() << '\n';
	...
	const int32_t nA3TWDIdx = oTempFileTreeFixture.findToWatchPath(oFofiModel, sBasePath + "/A3");
	EXPECT_TRUE(nA3TWDIdx < 0);
...
{
	std::cout << "FofiModel04 Tests:" << '\n';

	EXECUTE_TEST(fofi::testing::testDontFollowSymLinks());
	//
	std::cout << "FofiModel04 Tests successful!" << '\n';
	return 0;
}
